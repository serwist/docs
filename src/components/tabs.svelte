    ...
    <div role="tablist" aria-orientation="horizontal" class="flex w-full overflow-auto rounded-t-xl">
      {#each tabs as [tab, tabId] (tabId)}
        ...
        >
          {tab}
          {#if isActive}
            <span class="dark:bg-neutral-925 pointer-events-none absolute bottom-0 left-0 z-2 h-px w-full bg-white" aria-hidden="true"></span>
          {/if}
        </button>
      {/each}
    </div>
    <div class="pointer-events-none absolute bottom-0 left-0 z-1 h-px w-full bg-neutral-300 dark:bg-neutral-800" aria-hidden="true"></div>
  </div>
