<script lang="ts">
  ...
  import ExclamationMark from "./icons/exclamation-mark.svelte";
...
<div
  class={clsx(
    ...
    "my-3 border border-neutral-300 text-black shadow-md *:min-w-auto md:text-sm dark:border-neutral-800 dark:text-white",
    barColor
  )}
>
  <span class="size-5">
    <Icon width={20} height={20} class={clsx("rounded-full p-0.5 text-white dark:text-black", iconColor)} />
  </span>
  <span class="*:m-0!">
    {#if children}
      {@render children()}
